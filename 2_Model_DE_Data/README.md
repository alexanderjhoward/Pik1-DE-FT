...
## Method
The **esm2_average_feature_extraction.py** script was used to extract the embeddings of each sequence in our Avr-PikC and Avr-PikF data. These embeddings were averaged across the all amino acid positions to reduce the feature dimensionality.
...
The **esm2_finetuning.py** script was used to generate fine-tuned ESM-2 models from our Avr-PikC and Avr-PikF selection data on both a random train/validation split (using the same one used used in our embedding feature extraction) and the latent space train/validation split.
